...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderProviderSitemapsTable(sitemaps, count) {
        ...
        if (sitemaps.length === 0) {
            tbody.innerHTML = `<tr><td colspan="8" style="text-align: center !important; padding: 40px; color: #737373;">No provider site maps found.</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = sitemaps.map(s => `<tr><td>${s.providerName || '-'}</td><td>${s.metaProviderPageTitle ? s.metaProviderPageTitle.substring(0, 30) + (s.metaProviderPageTitle.length > 30 ? '...' : '') : '-'}</td><td>${s.metaProviderPageDescription ? s.metaProviderPageDescription.substring(0, 30) + (s.metaProviderPageDescription.length > 30 ? '...' : '') : '-'}</td><td>${s.providerDescription ? s.providerDescription.substring(0, 30) + (s.providerDescription.length > 30 ? '...' : '') : '-'}</td><td>${s.aboutProvider ? s.aboutProvider.substring(0, 30) + (s.aboutProvider.length > 30 ? '...' : '') : '-'}</td><td><a href="${s.publishedUrl}" target="_blank" style="color: #1570EF; text-decoration: none;">View</a></td><td>${formatDate(s.createdAt)}</td><td class="text-left"><div class="action-btn-container"><button class="action-btn" data-provider="${s.providerName}"><img src="/assets/icons/more-vert.svg" alt="More" /></button><div class="action-dropdown" id="psm-dropdown-${s.providerName}"><button class="dropdown-item-edit" data-provider="${s.providerName}"><img src="/assets/icons/edit-outline.svg" alt="" /><span>Edit</span></button><div class="dropdown-divider"></div><button class="dropdown-item-remove" data-provider="${s.providerName}"><img src="/assets/icons/delete.svg" alt="" /><span>Remove</span></button></div></div></td></tr>`).join("");
