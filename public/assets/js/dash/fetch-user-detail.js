...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (userNameEl) {
        userNameEl.textContent = user.userDetails?.fullName || user.name || "Unknown";
      }
      if (userEmailEl) {
        userEmailEl.textContent = user.userDetails?.email || user.email || "No email";
      }
      ...
      if (userDetails) {
        ...
        const paymentsPlanTextEl = document.querySelector(".payments-hd-sect-two");
        const paymentsDescriptionEl = document.querySelector(".payments-hd-sect-one-stle");
        if (userDetails.planId?.name) {
          ...
          if (paymentsDescriptionEl) {
            if (isFreePlan) {
              paymentsDescriptionEl.textContent = "Upgrade now to get started with a paid plan.";
            } else {
              paymentsDescriptionEl.textContent = "Manage your subscription or upgrade to a higher plan.";
            }
          }
        }
      }
